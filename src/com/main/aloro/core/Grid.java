...
public class Grid {
	...
	conwaysSimulator = new Thread(() -> {
	    simulationLoop();
	});

	conwaysSimulator.start();
    }
	...
	if (coord == Coordinate.X) {
	    ...
	    } else if (limit == Limit.MAX && initialPixel != ChunkManager.get().getChunkWidth()
		    * (ChunkManager.get().getNumberOfHorizontalChunks() - 1)) {
		ChunkManager.get().loadChunk(chunkId + 1);
	    }
	} else if (coord == Coordinate.Y) {
	    if (limit == Limit.MIN && initialPixel != 0) {
		ChunkManager.get().loadChunk(chunkId - ChunkManager.get().getNumberOfHorizontalChunks());
	    } else if (limit == Limit.MAX && initialPixel != ChunkManager.get().getChunkHeight()
		    * (ChunkManager.get().getNumberOfVerticalChunks() - 1)) {
		ChunkManager.get().loadChunk(chunkId + ChunkManager.get().getNumberOfHorizontalChunks());
	    }
	}
    }
